...
describe('project → suite → section → case CRUD', () => {
  ...
  it('reparents child sections instead of orphaning them on delete', async () => {
    const project = await request(app).post('/api/v1/projects').set(auth(adminToken)).send({ name: 'Reparent Test' });
    const suite = await request(app)
    ...
    const reparentedChild = sections.body.sections.find((s: { id: string }) => s.id === child.body.section.id);
    expect(reparentedChild.parentId).toBeNull();
  });
});
