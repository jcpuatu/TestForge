...
export function ComparisonForReferencesReport({ projectId }: { projectId: string }) {
  const [runIds, setRunIds] = useState<string[]>([]);
  const [sectionIds, setSectionIds] = useState<string[]>([]);

  const sectionsQuery = useProjectSections(projectId);
  const reportQuery = useQuery({
    queryKey: ['reports', 'results', 'comparison-for-references', projectId, runIds, sectionIds],
    queryFn: () => reportsApi.getComparisonForReferences(projectId, { runIds, sectionIds }),
  });
  const data = reportQuery.data;

  return (
    <ReportShell
      title="Comparison for References"
      description="Test results across a set of test runs, grouped by the requirement reference each case is linked to."
      filters={
        <>
      ...
      {data && (
        <div className="space-y-4">
          ...
          {data.references.length === 0 && (
            <p className="text-sm text-slate-500 dark:text-slate-400">No referenced cases in the selected runs.</p>
          )}
        </div>
      )}
    </ReportShell>
