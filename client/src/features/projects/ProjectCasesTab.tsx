...
export function ProjectCasesTab() {
  ...
  return (
    <div>
      <h1 className="mb-4 text-xl font-semibold text-slate-900">Test Cases</h1>
      {canManage && (
      ...
      <div className="space-y-2">
        {project.suites.map((suite) => (
          ...
          >
            ...
            {suite.description && <p className="mt-1 text-sm text-slate-500">{suite.description}</p>}
          </Link>
        ))}
        {project.suites.length === 0 && <p className="text-sm text-slate-500">No test suites yet.</p>}
      </div>
    </div>
  );
}
