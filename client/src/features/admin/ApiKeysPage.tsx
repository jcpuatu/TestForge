  ...
  return (
    <div>
      ...
      <p className="mb-6 text-sm text-slate-500">
        ...
        <a href="http://localhost:4000/api/v1/docs" target="_blank" rel="noreferrer" className="text-blue-600 hover:underline">
          API docs
        </a>
        .
      </p>
      ...
      <div className="divide-y divide-slate-200 rounded-lg border border-slate-200 bg-white">
        {keysQuery.data?.apiKeys.map((k) => (
          <div key={k.id} className="flex items-center justify-between p-3">
            ...
            {!k.revokedAt && (
              <button className="text-xs text-red-600 hover:underline" onClick={() => revokeKey.mutate(k.id)}>
                Revoke
              </button>
            )}
          </div>
        ))}
        {keysQuery.data?.apiKeys.length === 0 && <p className="p-3 text-sm text-slate-500">No API keys yet.</p>}
      </div>
    </div>
  );
}
